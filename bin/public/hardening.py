...
#
#  x("useradd " + name + " -G wheel,root")
#  error while x("passwd " + name):
#    pass
#
#  x("chmod +w /etc/sudoers")
#  general.set_config_property("/etc/sudoers",'^# %wheel.*ALL=\(ALL\).*ALL$',"%wheel        ALL=(ALL)       ALL")
#  x("chmod 0440 /etc/sudoers")
#
...
def _disable_ip6_support():
  ...
  general.set_config_property("/etc/modprobe.d/syco.conf", "^alias ipv6 off$","alias ipv6 off")
  general.set_config_property("/etc/modprobe.d/syco.conf", "^alias net-pf-10 off$","alias net-pf-10 off")
...
def configure_resolv_conf():
  ...
  general.set_config_property("/etc/resolv.conf", "domain.*", "domain " + config.general.get_resolv_domain())
  general.set_config_property("/etc/resolv.conf", "search.*", "search " + config.general.get_resolv_search())

def configure_localhost():
  app.print_verbose("Configure /etc/hosts")
  localhost =  "127.0.0.1"
  localhost += " " + gethostname() + "." + config.general.get_resolv_domain()
