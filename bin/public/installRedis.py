...
def install_redis(args):
  ...
  x("sed -i s/REDIS%%IDDCB%%/{0}}/g {1}keepalived.conf".format(socket.gethostname().upper(), KEEPALIVED_CONF_DIR))
  x("sed -i s/REDIS%%IDDCS%%/{0}}/g {1}keepalived.conf".format(socket.gethostname().lower(), KEEPALIVED_CONF_DIR))
